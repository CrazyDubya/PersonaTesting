...
Defines system prompts and content builders for each condition:
1. Baseline-MC (no persona, MCQ, short answer)
2. Shallow-Persona-MC (paper-style expert tag)
3. Deep-Persona-MC-Preamble (rich expert persona, long reasoning)
4. Deep-Persona-Open (rich expert persona, no multiple choice)
...
def get_persona_text(persona_type: str, domain: str) -> str:
    """Get the persona text based on persona type and domain."""
...
def build_mcq_content_with_short_answer(
    ...
    """
    Build content for MCQ with short answer format.
    Matches the original paper-style 'The correct answer is X'.
    """
...
def build_mcq_content_with_preamble(
    ...
    """
    Build content for MCQ with preamble and long reasoning.
    Includes explicit instruction for reasoning followed by final answer letter.
    """
    domain = question.subject or "the relevant subject"
    persona_text = get_persona_text(persona_type, domain)
    options_block_lines: List[str] = []
...
def build_open_content_persona_long(
    ...
    """
    Build content for open-ended questions with persona and long reasoning.
    """
    domain = question.subject or "the relevant subject"
    persona_text = get_persona_text(persona_type, domain)
...
def build_open_content_process_only(
    ...
    """
    Build content for open-ended questions with no persona but explicit reasoning.
    """
    content_parts: List[str] = []
    content_parts.append(
...
def build_user_content_for_condition(
    ...
    """
    Build user content based on the experimental condition.
    Routes to the appropriate content builder.
    """
...
def build_chat_messages(
    ...
    """
    Build messages suitable for a ChatCompletion-style API.
